...
% add noise
shift = [shift(1)*ones(numSamplesPerCondition,1);shift(2)*ones(numSamplesPerCondition,1)];
sigma = [sigma(1)*ones(numSamplesPerCondition,1);sigma(1)*ones(numSamplesPerCondition,1)];

profile = f.addNoise(profile,shift,sigma);
...
[TPR_p,FPR_p] = f.ROC(-log(pVal),DEind);
